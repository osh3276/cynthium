...
class SimulationResultsPanel(QWidget):
	...
	def _build(self):
		...
		all_fields = [_PATH_FIELDS, _SLOPE_FIELDS, _ENV_FIELDS]
	...
	def set_stats(
		...
		if not auto_available or auto_stats is None:
			status = "Simulation complete (manual path only)"
			auto_stats_final: dict[str, float] = {}
			self._apply_stats(self._manual_labels, manual_stats)
			self._apply_stats(self._auto_labels, auto_stats_final)
