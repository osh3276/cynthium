...
def simulate_rover_over_path(
    *,
    ...
    """Simulate the rover as a unicycle over a 3D path.
    ...
    if pts_xyz.shape[0] < 2:
        print(f"[simulate_rover_over_path] empty path — returning immediately ({time.perf_counter() - t_start:.3f}s)")
    ...
    for step in range(max_steps):
        ...
        # ── 2. Terrain slope ──
        pitch = _sample_pitch(x, y, pts_xyz, hint_idx=best_idx)
    ...
    print(
        f"[simulate_rover_over_path] {status} — "
